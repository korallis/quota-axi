...
/**
 * Ownership-stability order. The Muse CLI's own sign-in store comes first: it
 * is the credential the CLI itself sends to the key endpoint at startup, so it
 * names the subscription a live Muse session draws on. `META_API_KEY` is the
 * explicitly exported alternative and is consulted only when that store is
 * absent or its token is rejected.
 */
 ...
 * carries a freshly issued `api_key` and account fields; a JSON reviver drops
 * every key outside this set while the body is parsed, so those values never
 * reach a JavaScript object, an error, the cache, or any output.
 */
 ...
 * refreshes the token, and never stores the key the endpoint issues.
 */
...
export function createMuseAdapter(
  ...
  const dependencies: MuseDependencies = {
    sources: [createMuseAuthFileSource(), createMuseApiKeySource()],
    fetch: providerFetch,
 ...
 * /muse-code/key` with `{"onboard": false}`. The response carries an issued
 * API key alongside the subscription usage; only the usage keys survive
