...
- Providers are grouped by what quota-axi found on this machine, and the header counts each group, for example `4 live · 2 need attention · 10 not set up`. A provider with a reading, fresh or stale, is live. A provider without one whose sources found something (a credential that is expired, rejected, or waiting on a Keychain prompt, an installed tool that failed, or a request that failed) needs attention: it stays a dimmed full card right after the live cards, with its remedy, and is excluded from the live total. A provider is not set up only when every credential source it checks was skipped as absent (no file, environment variable, CLI, or running app). A provider that recorded no sources at all still needs attention, so absence is never assumed, and so does one whose skip could not establish absence either way, such as a Copilot CLI configuration that names no account quota-axi can confirm. A GitHub CLI login alone, or one Copilot refused, is not evidence of Copilot access and leaves Copilot not set up rather than broken; a request through it that failed transiently keeps Copilot in view.
