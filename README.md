...
quota-axi reports local Claude, Codex, Cursor, GitHub Copilot, Grok, Kimi, Z.AI, Alibaba, OpenCode Go, Antigravity (`agy`), Command Code, MiniMax, MiMo, DeepSeek, OpenRouter, ElevenLabs, and Muse quota windows in one [AXI](https://axi.md)-shaped call.
...
| Flag                                                                                                                                        | Description                                                               |
| ------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `--provider claude,codex,cursor,copilot,grok,kimi,zai,agy,alibaba,opencode-go,commandcode,minimax,mimo,deepseek,openrouter,elevenlabs,muse` | Scope providers                                                           |
...
| Muse                   | Reads the Muse CLI's `auth.json` login (`providers.meta.access_token`), then an exported `META_API_KEY`, and reports the subscription's `five_hour` (`subs_usage.window` when `window_duration_mins` is 300) and `weekly` (`subs_usage.weekly`) windows from `used_percent` and epoch-second `resets_at`, via `POST https://api.meta.ai/muse-code/key` - the request the Muse CLI makes at startup. Both windows jointly bound `all_models`. A window of any other length is named `window` with its own duration and stays unresolved; an entry without a usable `used_percent` is named in `untrustedWindowIds`; an inactive subscription reports no window. Because that endpoint also issues an API key, requests are rate-bounded per credential; see [Muse provider notes](#provider-notes).                                                                                                                                                                                                            |
...
| Muse | The Muse CLI's `${XDG_CONFIG_HOME:-~/.config}/muse/auth.json` `providers.meta.access_token` first, then `$META_API_KEY`, each only when it holds a usable literal credential. Only the access token is read from `auth.json`; a sibling `refresh_token` is checked for presence only. Neither carries a stored expiry quota-axi reads, so both are probed in declared order. |
...
- Muse (Meta's Muse Code CLI) exposes subscription usage only on `POST https://api.meta.ai/muse-code/key` with `{"onboard": false}`, the request the Muse CLI itself sends at every startup. quota-axi sends that same request with the same bearer and reads `subs_usage.window`, `subs_usage.weekly`, `is_subs_active`, and `subs_tier_name` (as `plan`).
- That endpoint also issues an API key in its response. quota-axi never keeps it: a JSON reviver drops every response key outside the usage fields while the body is parsed, so the issued key and any account fields (email, name, payment method) never reach an object, an error, the cache, or any output mode. No response body is ever quoted in an error. quota-axi never writes that key, or anything else, to Muse's `auth.json`.
- Because every request issues a key, requests are bounded per credential: at most one request per credential per five minutes, whatever its outcome, across every quota-axi process sharing the cache directory. The request is recorded as `pending` in the [key-endpoint ledger](#cache) before it leaves, so a timeout or crash still counts. Inside the interval nothing is sent: the reading that request produced is served from its own context-matched snapshot as `status: fresh` with `source: cache` and its original `refreshedAt`; a rejection is replayed as a rejection; and anything else reads status `unavailable` with error `muse_read_deferred` and a `retryAfter` naming when the next request may leave, falling back to that credential's older snapshot as stale when one is still inside its windows. The interval equals the `--tui` default refresh, so a live report sends at most one request per refresh, and a burst of agent calls sends one in total. `quota-axi auth` never contacts the endpoint. Concurrent processes that pass the gate at the same instant can each send one request; an unwritable cache directory cannot record the interval, and each invocation then still sends at most one request.
- `auth.json` is read for `providers.meta.access_token` only; a sibling `refresh_token` is checked for presence and its value is dropped while the file is parsed. HTTP 401 on a token stored beside a refresh token is soft expiry (`authStatus: expired_refreshable`, status `unavailable`, cache preserved), because the Muse CLI renews its own session at startup; without one it is sign-out. HTTP 403 is not sign-out evidence. Muse has no delegated refresh, and `ProviderOptions.refreshCredentials` is ignored.
- Snapshots are cache-scoped by an opaque SHA-256 of the answering source plus a one-way digest of its credential, so a token the Muse CLI has since refreshed starts a new identity - a cache miss and a new request, never a cross-attribution.
...
| Cursor, GitHub Copilot, Kimi, Z.AI, Alibaba, OpenCode Go, Antigravity, Command Code, MiniMax, MiMo, DeepSeek, OpenRouter, ElevenLabs, Muse | none                              | read-only; see the per-provider notes below           |
...
| Muse cache provenance                  | Follows the [Muse provider rules](#provider-notes): each snapshot carries an opaque SHA-256 of the answering source plus a one-way digest of the credential that produced it, and both interval reuse and stale fallback require that identity to match. A reading with no usable credential neither caches nor clears the record.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| Muse key-endpoint ledger               | Lives alongside the quota cache as `muse-key-reads.json` (`0600`). It maps each Muse credential's opaque context digest to the time and outcome class (`pending`, `quota`, `rejected`, `transient`) of the last key-endpoint request, holds no credential material or account data, and drops entries older than the five-minute interval on each write.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
